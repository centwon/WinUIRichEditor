...
### Fixed — 큰 줄간격에서 ↓ 이동이 같은 줄에 갇히던 회귀 (2026-07-31, 3차 전수조사)

- **캐럿 높이를 이동에 쓰던 것을 줄 높이로 분리했다.** 같은 날 캐럿을 baseline 기준·글자 높이로 바꾼
  수정이 넣은 회귀다 — 세로 이동 두 곳이 "캐럿 바닥 + 넛지"로 다음 줄을 찾는데, 캐럿이 이제 줄 상자보다
  짧아져 넛지가 **같은 줄 상자 안**에 떨어질 수 있었다. 간격은 `0.2 × (줄상자 − 글자높이)`라 100%에서는
  0이지만 **200%에서 ~3px, 300%에서 ~6px**로 벌어진다: 셀 안 이동(`+2` 넛지)은 200%부터, 문단 간
  이동(`+14` 넛지)은 300% 근처부터 걸린다. 그리기용 캐럿 높이와 **넘어가야 할 줄 높이는 이제 다른
  개념**이므로, `CaretInLayout`/`CaretToDocPoint`가 줄 상자 바닥을 함께 돌려주고 이동 경로만 그것을 쓴다.
  **핵심 계약**: 세로 이동 probe는 캐럿이 아니라 **줄**을 넘어야 한다 — `LineBottom`을 쓸 것.
  100%에서는 두 값이 같아 기존 동작과 동일하다.
- 스크롤(`ScrollCaretIntoView`)은 그대로 캐럿의 시각 범위를 쓴다 — 줄 상자 전체가 아니라 캐럿을
  보이게 하는 것이 맞다.
