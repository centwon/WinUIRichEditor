...
- **쓰는 쪽이 없었다** — Word/HWP로 내보내면 머리말이 조용히 사라졌다. 이제 문서 영역에
  `{\header …}`/`{ooter …}`를 쓴다(Word가 두는 자리, 본문보다 앞).
...
> ③ **구분자가 내용이 되어 쌓였다** — 우리가 쓰는 `\chpgn / {ield NUMPAGES}`의 `" / "`가 꼬리말
...
이제 Word가 자기 목록에 쓰는 것과 같은 형태를 쓴다 — `i-360`(마커를 내어쓰기) + `\li<거터>`(글은
거터에서 시작) + `	x<거터>`(탭이 거기서 멈춘다).

> ⚠️ **거터는 `\li` 안에 들어가고, 이 리더는 `\li`를 저자의 들여쓰기로 읽는다.** 보정이 없으면 목록
> 항목의 들여쓰기가 **저장할 때마다 거터만큼 자란다.** 그래서 `{\*rlvl<level>}` 태그를 마커 태그
...
- **HWP에서 줄간격이 바뀌었다** — 간격을 안 쓰면 "리더의 기본값"이고 HWP의 기본은 160%다. 이제 미지정도
  ...
  > ⚠ 그리고 `{\*rsl}` 태그를 함께 단다. 이 모델에서 **미지정과 1.0은 같은 것이 아니기 때문**이다 —
...
`{\*rsl}` 그룹을 붙이자 공백이 그룹 뒤로 밀려 **내용이 됐다**(" 항목"). 태그는 공백 다음에 온다.
