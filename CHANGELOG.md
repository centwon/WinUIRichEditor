...
- **빈 줄이 브라우저에서 보이지 않았다.** 빈 문단(과 빈 목록 항목)을 내용 없는 `<p>`로 써서 높이가 0이었다. 이제
  `<br/>`을 함께 쓰고, 이 편집기는 표식(`data-are-empty`)이 있으면 그 `<br/>`을 내용으로 읽지 않는다 — 한 줄이
  두 줄로 불어나지 않는다. 표식 없는 외부 HTML의 빈 요소는 전처럼 버리고, 외부의 `<p><br></p>`은 전처럼 빈 줄이다.
- **셀 속 그림이 캡션 아래가 아니라 옆에 붙었다.** 셀에 문단 하나와 그림이 있으면 문단을 태그 없이 썼고, `<img>`는
...
릴리스의 무게는 검증에 있다: 테스트 **391 → 815**, 그 과정에서 결함 40여 건을 고쳤다. 상류
AvaloniaRichEditor와 양방향 백포트(상류 PR #26~#44).

### ⚠ 올릴 때 확인할 것 (동작 변경 — 공개 표면은 그대로)

- **줄 간격이 HWP 기준이 됐다.** `Paragraph.LineSpacing`은 이제 "글자 크기 × 값"(1.6 = 160%)이고, 설정 없는 문단은
  **160%**로 그려진다(전에는 폰트 고유 높이 ≈120%). 문단 기본 아래 여백은 10px → **0**(저장된 파일은 적혀 있으므로
  그대로, 필드 없는 JSON·새 문단·붙여넣은 문단만 0). **저장 형식은 그대로지만 기존 문서가 더 넓게 보인다.**
- **제목의 굵게·크기가 글자 속성이 됐다.** 렌더러가 강제하지 않으므로 제목 안에서 굵게를 끄거나 크기를 바꿀 수 있고,
  제목을 (같은 수준이라도) 다시 적용하면 제목 모양으로 되돌아간다. 옛 문서는 편집기가 받을 때 한 번 변환하고
  (`"HeadingFormat": 1`이 추가된다) **직렬화기는 변환하지 않는다** — 편집기 없이 옛 파일을 HTML/RTF로 바로 바꾸는
  호스트는 제목이 글자 속성 그대로 나간다. 상류는 여전히 강제한다(여기서 갈라진다).
- **굵게·기울임·밑줄·취소선 토글이 Word 방식**: 대상 글자가 전부 그 서식이면 끄고, 하나라도 아니면 모두 켠다
  (전에는 런마다 뒤집었다). 상류는 아직 옛 방식이다.
- **인쇄와 `SavePdf`가 글자로 나간다**(전에는 페이지마다 비트맵). 프린터 해상도로 나가고 PDF에서 글자를 선택·검색할 수
  있다. `dpi` 인자는 소스 호환용으로 남았고 대체 경로에만 쓰인다.
- **문서가 아닌 입력을 거부한다.** `LoadJson`/`LoadJsonAsync`는 `Blocks` 없는 JSON을 `JsonException`,
  `LoadPackageAsync`·`DocumentPackage.Load`는 `document.json` 없는 zip을 `InvalidDataException`으로 던지고 **열린 문서를
  지킨다**(전에는 조용히 빈 문서로 바꾸고 "저장됨"으로 표시해 다음 저장이 원본을 덮었다). 리터럴 `null`만 빈 문서다.
  공개 `DocumentSerializer.Deserialize`는 관대한 채다.
- **페이지 설정이 호스트 기본값에서 시작한다** — 설정 없는 문서·새 문서가 직전 문서의 용지·방향·머리글을 물려받지 않는다.
- **이미지·표를 끈 에디터는 들어오는 내용을 적응시킨다** — 리치 붙여넣기와 `InsertHtml`에서 이미지는 버리고 표는 셀
  글자로 푼다(전에는 `AllowRichPaste`만 봐서 그대로 들어왔다). `Load*`와 문서 안 드래그는 영향 없다.
- **링크는 http/https만 연다**(보안, 아래 참조). `mailto:`도 열지 않는다.
- **언두 이력의 예산 지표가 바뀌었다** — 글자 수가 아니라 요소 수 기준(계측값). 큰 문서의 이력은 더 일찍 잘리고,
  텍스트가 긴 문서의 이력은 더 오래 남는다.
- 영어 UI에서 셀 세로 정렬 "Center" → **"Middle"**(한국어는 "가운데" 그대로).

### Security

- **문서 속 링크가 웹 주소가 아니어도 실행됐다.** 링크 주소는 붙여넣은 HTML·RTF에서 그대로 들어오므로, `ms-msdt:` ·
  `search-ms:` · `ms-settings:` 같은 프로토콜 링크가 Ctrl+클릭(뷰어에서는 클릭) 한 번에 확인 없이 핸들러를 실행했다.
  이제 http/https만 연다(상류가 이미 쓰던 규칙). 열 수 없는 링크는 메뉴의 "링크 열기"가 비활성이다.
- `AllowLocalFileImages=false`인데도 `LoadHtml`/`InsertHtml`이 `file:` 이미지를 읽던 것, `AllowImages=false`인데도
  `InsertImageBlock`/`InsertImageBytes`가 이미지를 넣던 것을 막았다.

### Added

- **벡터 인쇄·PDF** — Win2D `CanvasPrintDocument`로 프린터의 드로잉 세션에 직접 그리고, `SavePdf`는 "Microsoft Print to
  PDF"로 대화상자 없이 글자 PDF를 만든다(프린터·스풀러가 없으면 예전 비트맵 경로). **Native AOT에서도 인쇄 대화상자가
  열린다** — `IPrintManagerInterop`을 COM vtable로 직접 불러 리플렉션 투영을 피했다.
- **표·그림을 끌어서 옮기기** — 표의 왼쪽·위쪽 테두리나 그림을 끌면 그 자리로 옮겨지고, 놓을 때 Ctrl을 누르고 있으면 복사된다.
- **그림을 가로·세로로 따로 늘리는 손잡이** — 오른쪽 아래 모서리(비율 유지) · 오른쪽 가운데(너비) · 아래 가운데(높이),
  커서도 ↘·↔·↕로 바뀐다(Word와 같은 배치). 끌기 한 번은 되돌리기 한 단계다.
- **한 셀 선택(F5)과 Shift+방향키 셀 블록** — 캐럿이 있는 셀을 셀 블록으로 잡고, Shift+방향키로 셀 단위로 넓힌다.
  복사는 1×1 표로 담긴다. 상류와 셀 선택 규칙을 통일했다.
- **찾기 UI** — `FindRequested` · `LastFindQuery` · `FindAgain`, Ctrl+F/H · F3 · Shift+F3, `RichEditorView`의 찾기 막대
  (`ShowBuiltInFindBar`). 툴바의 찾기 버튼은 받는 쪽이 있을 때만 보인다.
- **XAML에서 바인딩할 수 있는 툴바·뷰 속성 7개**(유일한 공개 API 추가) —
  `RichEditorToolbar.TargetProperty` · `ToolbarLevelProperty` · `ShowPageControlsProperty` · `ShowFileActionsProperty`,
  `RichEditorView.ShowStatusBarProperty` · `ShowFileActionsProperty` · `ShowBuiltInFindBarProperty`. 편집기 상태를 전달하는
  뷰 속성(`Document` · `IsReadOnly` · `ZoomFactor`)은 편집기 쪽 속성에 바인딩한다(이미 의존성 속성).
- **툴바 아이콘이 벡터 그림**이 됐다(글꼴 글리프 의존 제거).

### Changed

- **캐럿 서식 보고가 다음 타이핑과 화면을 그대로 말한다** — 타이핑 서식은 캐럿 앞의 가장 가까운 글자에서, 이미지는
  건너뛰고, 없으면 뒤의 글자에서(Word 방식). 삽입과 툴바 보고가 같은 결정(`TypingSource`)을 쓴다 — 따로 있던 동안
  이미지 옆 4개 위치에서 툴바와 타이핑이 달랐다. 렌더러가 강제하는 서식(링크의 밑줄·파랑)은 그려진 대로 보고하고,
  그것에 대한 토글은 변경도 언두 단계도 만들지 않는다.
- **표를 잡으면 표 메뉴** — 표 경계를 우클릭하면 항목·순서까지 상류와 같은 표 메뉴가 뜨고, 복사가 표를 가져간다.
  테두리로 잡은 표는 셀 항목을 흐리게, 셀 안 글자 메뉴에는 "셀 선택"이 있다. 읽기 전용에서도 표 복사·모두 선택이 된다.
- **우클릭 메뉴가 세로로 조밀해졌다**(상류와 같은 여백).
- **선택한 그림의 손잡이가 그 아래 표 경계보다 우선한다** — 셀을 꽉 채운 그림의 손잡이를 쓸 수 있다(상류와 같은 순서).
- **그림을 원본 크기가 아니라 그려지는 크기로 디코드한다**(비율이 다르면 사각형을 덮는 가장 작은 크기, 원본보다 크게는 안 함).

### Fixed

**캐럿 · 입력**
- 소프트 줄바꿈(Shift+Enter)으로 끝나는 줄의 오른쪽을 클릭하면 캐럿이 아래 줄로 갔고, 그 빈 줄에서 ↑가 안 먹었다.
- **AltGr 문자를 Ctrl+Alt 단축키가 가로챘다** — AltGr는 Windows에 Ctrl+Alt로 들어와, 독일어 AltGr+2(²)·AltGr+3(³)이
  제목 단축키에, 폴란드어 AltGr+C(ć)가 복사에 먹혔다(Word와 같은 충돌). 이제 그 조합이 현재 자판에서 문자를 만들면
  문자로 입력된다. AltGr가 없는 자판(한국어·영어)은 그대로다.
- 제목 안에서 크기를 직접 지정한 글자의 캐럿 크기. 툴바가 그려진 것과 다른 글자 크기를 보고해 "크게"가 글자를 줄이던 것.
- 서식 복사(format painter)가 문서를 바꾼 뒤에도 켜져 있던 것.
- **Native AOT 전용**: 캐럿이 글자 높이가 아니라 줄 상자 높이로 그려지고, 소프트 줄바꿈 뒤의 빈 줄을 방향키가
  건너뛰던 것(줄 메트릭 대체 경로에 기준선이 없었다).

**표**
- 인라인 표 셀에서 Ctrl+A가 바깥 표를 건너뛰었다.
- 병합 셀에서 "아래에 행 삽입"이 병합 안쪽에 행을 넣었고, 행/열 명령이 캐럿을 보이지 않는 칸에 뒀다.
- **표 크기 끌기**: 표가 셀 밖으로 자랐고, 끌기 중 연 문서가 "수정됨"이 됐다.
- **표 그리기 모드**: 모드가 켜진 채 문서를 열거나 되돌리면 다음 클릭이 새 문서에 표를 넣었고, 포인터를 잃어도 모드가
  남았다. 셀 안에서 그린 표가 셀보다 넓어지던 것도 셀 폭으로 제한한다.
- 표를 셀 안에 붙여넣으면 그 셀에 중첩된다(고정).

**그림**
- 원본과 비율이 다르게 늘린 그림이 흐리게 그려지고 **화면에 있는 동안 끝없이 다시 디코드**됐다(인쇄는 페이지마다 동기 디코드).
- 많이 줄여 그린 그림의 줄무늬·작은 글자가 깨졌다(쌍선형 → Anisotropic 보간).
- 화면에 아직 그려지지 않은 그림이 인쇄·PDF에 "불러오는 중" 자리표시로 찍혔다.
- Ctrl을 누른 채 선택 영역을 끌면 복사가 아니라 새 선택이 시작됐다.

**링크**
- 링크 클릭·우클릭 판정이 반 글자 어긋났다. 자동 링크가 Tab 키와 괄호로 끝나는 주소에서 어긋났다.
- 링크 삽입 대화상자가 입력을 기다리지 않았고, 빈 곳에서는 주소를 링크로 넣지 않았다.

**파일 · 클립보드 · 모델**
- **BOM이 붙은 파일을 가져오지 못했다** — 메모장·Visual Studio·PowerShell이 붙이는 BOM 때문에 형식 판별이 빗나가
  UTF-8 BOM의 HTML·JSON·RTF와 UTF-16 파일이 조용히 실패했다. 이제 BOM으로 인코딩을 정한다.
- 파일의 병합값 정규화, 이미지 풀의 빈 항목, 연속 용지 설정 보존.
- **되돌리기 기록이 지운 그림의 원본 바이트를 메모리 예산 밖에서 붙잡았다.**
- 편집으로 지운 그림의 디코드된 비트맵이 문서를 바꿀 때까지 남았다.

**컨트롤 · 호스트**
- **두 번째 UI 스레드의 창이 첫 그리기에 죽었다**(브러시를 스레드마다 만들도록).
- 호스트가 듣는 이벤트: 선택 종류가 바뀔 때 `SelectionChanged`, 편집 뒤 `IsModifiedChanged`가 오지 않았다.
- 툴바가 기능 플래그를 늦게·일부만 반영했고, 실행 중 읽기 전용으로 바꾸면 다음 입력까지 편집 툴바(가져오기 포함)가 남았다.
- 툴바의 글꼴 목록 변경이 반영되지 않았고, 이미지 버튼에 폴백이 없었다.
- 찾기 막대의 검색 상자에서 F3·Shift+F3가 동작하지 않았고(포커스가 상자에 남아서), 툴바의 찾기 버튼이 받는 쪽 없이도 보였다.

### Performance

- **그림 복사의 일시 할당 352MB → 160MB**(10MB 사진 기준) — HTML/RTF 작성기가 페이로드를 여러 번 문자열로 만들던 것을
  한 builder에 조각 단위로 쓰도록. 출력은 바이트 단위로 동일하고 파일 내보내기에도 적용된다.
- 언두 예산이 실제 비용을 재게 됐다(요소 수 × 155바이트 — 계측값, 오차 1% 이내). 옛 추산은 텍스트가 많은 문서를 25배
  과다 계상하고 일반 문서를 1.5배 과소 계상했다.

### Tests

391 → **815**. 새 축: 줌 좌표 수학 · 편집 명령 25개 · 기능 플래그 게이트 약 63곳 · 문서 API 래퍼(사용자 파일을 지키는
계약) · 하이퍼링크 · 캐럿 서식 보고 · 호스트 이벤트 · 표 크기 끌기 · 그림 손잡이 · 단축키 표 · 찾기 UI · 바인딩 가능한
속성. 랜덤 편집열 퍼즈에 명령 단계와 그림 캐시 검사를 더했다. 모든 수정은 **반증**(수정을 되돌려 새 테스트가 빨개지는지)으로
확인했다 — 그 과정에서 공허한 테스트 둘을 잡았다.
