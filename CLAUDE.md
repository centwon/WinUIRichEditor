...
공개 API가 동결**됐다(SemVer, 표면은 `PublicAPI.Shipped.txt`가 추적). 현재 **`1.2.0`**(2026-09-20,
동결 이후 첫 기능 릴리스 — 추가만, breaking 없음).

**상류와의 관계**: 이제 포팅-원본이 아니라 **대등한 수렴 peer**다
(`C:\Users\centw\source\repos\AvaloniaRichEditor`). 양방향으로 개선을 주고받는다(최근 상류 PR #26~#44).
...
정독이 **아니라** 아래이며, 새 작업에도 그대로 쓸 것(레시피와 함정은 `Project_Roadmap.md`):
...
3. **랜덤 편집열 퍼즈**(`DocumentFuzzTests`) — 정독이 못 보는 "안 시험한 조합"을 잡는다.
   포매터를 건드리면 **시드를 넓혀** 돌릴 것(기본 400, 결함 하나는 200시드에서만 나왔다).
4. **반증** — 수정을 되돌려 새 테스트가 **빨강이 되는지** 확인. 공허한 테스트를 여러 번 잡았다.
5. **AOT 동등성 스윕**(`tools/fault-sweep.ps1`) — **AOT는 렌더돼도 동등하지 않을 수 있다**. 1.1.0이
   그래서 깨진 채 나갔다(줄 메트릭). 게시 전 생략 금지.
6. **실기 확인** — 컨트롤 계층(포커스·캐럿·포인터)은 자동 검증이 없어 사람이 봐야 한다.
   이것이 1.3 최우선 항목(상호작용 테스트 인프라)인 이유다 — 1.2.0에서도 포인터 순서 회귀 하나를
   좌표 테스트가 놓치고 실기가 잡았다.
