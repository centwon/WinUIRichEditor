...
/// <summary>Color parsing/formatting helpers. WinRT's <see cref="Color"/> has no <c>Parse</c> (Avalonia's
/// did), so this provides the union the formatters need: <c>#RGB</c>/<c>#RRGGBB</c>/<c>#AARRGGBB</c> hex,
/// <c>rgb()/rgba()</c>, and a small set of CSS named colors. <see cref="ToHexArgb"/> matches
/// <c>Color.ToString()</c> ("#AARRGGBB") so the JSON serializer round-trips; <see cref="ToCss"/> emits
...
internal static class ColorUtil
{
    ...
    public static Color? Parse(string? value)
    {
        ...
        var rgb = RgbRx.Match(v);
        if (rgb.Success)
        {
            byte r = ClampByte(rgb.Groups[1].Value);
        ...
        if (v.StartsWith("#", StringComparison.Ordinal)) return ParseHex(v.Substring(1));

        return Named(v.ToLowerInvariant());
    }
    ...
    private static Color? ParseHex(string h)
    {
        try
        {
            switch (h.Length)
            {
                case 3: // RGB shorthand -> RRGGBB
                    return Color.FromArgb(255, Dup(h[0]), Dup(h[1]), Dup(h[2]));
                case 6:
                    return Color.FromArgb(255, Hex(h, 0), Hex(h, 2), Hex(h, 4));
                case 8: // AARRGGBB (Color.ToString() form)
    ...
    private static Color? Named(string name)
    {
        ...
        return Css.TryGetValue(name, out uint rgb)
            ? Color.FromArgb(255, (byte)(rgb >> 16), (byte)(rgb >> 8), (byte)rgb)
            : (Color?)null;
    }

    // Full CSS/X11 named colors (lowercase keys, 0xRRGGBB), matching what Avalonia's Color.Parse accepts —
    // original .flow/JSON files store colors as these names (e.g. "Blue", "Coral"), so the port must read
    // them all to load without dropping colors. "transparent" is handled above.
    private static readonly System.Collections.Generic.Dictionary<string, uint> Css = new()
    {
    ...
    /// <summary>"#AARRGGBB" — matches <c>Windows.UI.Color.ToString()</c>, used as the JSON storage form.</summary>
    public static string ToHexArgb(Color c) => $"#{c.A:X2}{c.R:X2}{c.G:X2}{c.B:X2}";

    /// <summary>CSS-valid color: <c>#RRGGBB</c> when opaque, else <c>rgba()</c>.</summary>
