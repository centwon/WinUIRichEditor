...
**다음 단계**: 여전히 **포인터/키 주입은 없다**(WinUI는 `PointerRoutedEventArgs` 생성자가 internal이라
상류 `InteractionHost`처럼 이벤트를 합성할 수 없다). 남은 무검증 영역은 **이벤트 args 뒤에 가려진 게이트들**
(메뉴의 읽기 전용 분기, IME `TextUpdating`, 포인터 히트테스트)이다. 셋 다 그 분기를 args에서 분리해
**순수 함수로 빼면** 도달 가능해진다 — 외부 리뷰의 "히트테스트/세로이동 분리" 제안과 같은 처방이고,
이제는 그걸 받쳐 줄 하네스가 있다.
