...
**남은 항목**(1.3.0 게시 전):
- **AOT 동등성 스윕** — 1.2.0 이후 안 돌렸다. 새 `PageMarginProperty`는 사용자 정의 `record struct`를 DP 값으로
  담는다(WinRT 경계). `tools/fault-sweep.ps1`에 **여백 변경 단계가 없다** — 넣고 돌릴 것.
- **퍼즈 생성기가 페이지 설정·여백·`Block.AutoTopMargin`(NaN)을 만들지 않는다** — 이번 주기에 추가된 직렬화 필드가
  퍼즈 밖이다.
- **실기 확인**: 여백 픽커, 쪽 보기의 표·그림 선택 테두리.
