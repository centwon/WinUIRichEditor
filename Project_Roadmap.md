...
2. **시스템 설정 반영** — 캐럿 깜빡임 `530ms`와 더블클릭 `500ms`가 하드코딩이다
   ([`RichEditor.Input.cs:41,73`](src/WinUIRichEditor/Controls/RichEditor.Input.cs)).
   `GetCaretBlinkTime()`/`GetDoubleClickTime()`을 읽어야 한다 — 취향이 아니라 **접근성**이다
   (전자는 사용자가 깜빡임을 끄면 `INFINITE`를 돌려주는데 지금은 무시하고 계속 깜빡인다).
3. **삼킨 예외의 관측 수단** — 라이브러리 전역 약 60곳이 의도적 폴백으로 예외를 삼킨다(대부분 주석으로
   의도가 적혀 있다). 폴백 동작은 옳지만 **소비자가 그것을 볼 방법이 하나도 없다**. 옵트인 진단 훅 하나.
4. 아래 "알려진 한계"의 미수정 항목들.
...
- **수용·완료**: RTF 파싱 실패가 관측 불가 → 위 CHANGELOG `[Unreleased]` 참조. 리뷰에서 **유일하게
  데이터 손실로 이어지던** 항목이다.
- **수용·대기**: 위 1.1 항목 2·3, 그리고 히트테스트/세로이동을 순수 함수로 분리(**단, 1.1 항목 1의
