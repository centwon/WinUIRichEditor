...
⚠️ **실기 확인이 밀려 있다**: 시스템 입력 타이밍(아래 완료 항목)은 **개발 기계가 Windows 기본값
(530/500)이라 눈으로는 아무 차이가 없다.** 검증하려면 설정을 실제로 바꿔야 한다 — 방법은 아래
"재사용할 검증 레시피"에 적어 뒀다.
...
- **시스템 입력 타이밍 확인**: 기본값(530/500) 기계에서는 변화가 보이지 않으므로 설정을 바꿔서 본다.
  ...
  **레지스트리를 고쳤으면 재로그인해야 `GetCaretBlinkTime`이 새 값을 돌려준다**(USER가 세션 시작 시
  캐시한다). 컨트롤은 **포커스를 얻을 때** 다시 읽으므로, 앱을 띄운 채 설정만 바꿨다면 에디터 밖을
  클릭했다가 다시 들어오면 반영된다.
