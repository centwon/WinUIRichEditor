...
**다음 단계**: 지금은 명령·API·트리 수준이고 **포인터/키 주입은 아직 없다**(WinUI는 `PointerRoutedEventArgs`
생성자가 internal이라 상류 `InteractionHost`처럼 이벤트를 합성할 수 없다). 캐럿 기하·세로 이동·히트테스트는
내부 메서드를 직접 부르는 쪽이 현실적이고, 그것이 외부 리뷰의 "히트테스트/세로이동을 순수 함수로 분리"
제안을 흡수할 자리다.
