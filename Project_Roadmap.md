...
상류 `InteractionHost`처럼 이벤트를 합성할 수 없다). 남은 무검증 영역은 **페이지네이션**과, 이벤트 args
뒤에 가려진 게이트들(메뉴의 읽기 전용 분기, IME `TextUpdating`)이다. 후자는 그 분기들을 args에서 분리해
**순수 함수로 빼면** 도달 가능해진다 — 외부 리뷰의 "히트테스트/세로이동 분리" 제안과 같은 처방이다.
