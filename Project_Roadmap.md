...
2. **삼킨 예외의 관측 수단** — 라이브러리 전역 약 60곳이 의도적 폴백으로 예외를 삼킨다(대부분 주석으로
   의도가 적혀 있다). 폴백 동작은 옳지만 **소비자가 그것을 볼 방법이 하나도 없다**. 옵트인 진단 훅 하나.
3. 아래 "알려진 한계"의 미수정 항목들.
...
- **수용·대기**: 위 1.1 항목 2, 그리고 히트테스트/세로이동을 순수 함수로 분리(**단, 1.1 항목 1의
