...
> **잔여 큰 항목**: 원본(AvaloniaRichEditor 0.8.0) 대비 패리티 갭 분석 미실시 — 다음 작업 후보 1순위.
...
> - **뷰어 표 테두리**: 읽기 전용은 **캐럿을 아예 안 그리므로**(`DrawCaret` 조기 반환) "캐럿이 놓이는지"로는
>   확인할 수 없다. 표 좌/상단 테두리에서 ① 호버 시 이동(✥) 커서가 뜨지 않고 I-beam 유지 ② 클릭해도
>   파란 선택 테두리가 안 생김 ③ 드래그하면 텍스트 선택 하이라이트가 정상 — 이 셋으로 확인한다.
